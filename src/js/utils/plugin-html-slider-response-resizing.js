var jsPsychHtmlSliderResponseResizing = (function (jspsych) {
  'use strict';

  var version = "2.1.0";

  const info = {
    name: "html-slider-response-resizing",
    version,
    parameters: {
  ...
  class HtmlSliderResponsePlugin {
    ...
    trial(display_element, trial) {
      ...
      html += '">';
      html += '<input type="range" class="jspsych-slider" value="' + trial.slider_start + '" min="' + trial.min + '" max="' + trial.max + '" step="' + trial.step + '" style="width: 100%;" id="jspsych-html-slider-response-response"></input>';
      html += "<div>";
      ...
      // Function to update stimulus size based on slider value
      const updateStimulusSize = (value) => {
        // Map slider value directly to percentage scale
        // If slider is 0-100, then 0 -> 0%, 50 -> 50%, 100 -> 100%
        const scale = value / 100;
        const stimulusElement = display_element.querySelector("#jspsych-html-slider-response-stimulus");
        stimulusElement.style.transform = `scale(${scale})`;
      };
      ...
      if (trial.require_movement) {
        ...
        sliderElement.addEventListener("mousedown", enable_button);
        sliderElement.addEventListener("touchstart", enable_button);
        sliderElement.addEventListener("change", enable_button);
      }
      ...
      display_element.querySelector("#jspsych-html-slider-response-next").addEventListener("click", () => {
        ...
        response.response = sliderElement.valueAsNumber;
